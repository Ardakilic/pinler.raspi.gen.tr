jQuery(document).ready(function(){
	...
	History.Adapter.bind(window,'statechange',function(){
		...
		var url = State.url.replace('http://pinler.raspi.gen.tr','');
		if( url == '/' )
		{
			jQuery('h1').click();
		}
	...
	jQuery('h1').on('click',function(){
		...
		History.pushState({pin:homepage,url:'http://pinler.raspi.gen.tr'}, 'Raspberry Pi Pinout', '/');
	});

	url = window.location.href.replace('http://pinler.raspi.gen.tr/','/');
	console.log(url);
	jQuery('a[href="' + url + '"]').click();
});